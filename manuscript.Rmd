...
*i* indicates an individual item, and *R* indicates the reference group [@nye2011]. Therefore, an individual score is predicted by the intercept of the item ($\tau$) added to the item loading ($\lambda$) times the latent variable score ($\xi$). @nye2011 demonstrated that the terminology from IRT and DIF can be used to create a measure of item functioning differences within CFA, $d_{MACS}$. $d_{MACS}$ is then the difference of reference group versus focal group (e.g., group 1 versus group 2) divided by a pooled standard deviation similar to formulas for *d* proposed by @cohen2013. Therefore, if $\tau$ or $\lambda$ are individually (or both) different across groups, it will impact all the predicted $X_{iR}$ scores, and thus, impact $d_{MACS}$ values. While $d_{MACS}$ represents a necessary step for development of effect sizes within CFA, it does not separate the differences in parameters between groups in a way that can be paired with traditional MGCFA testing and partial invariance. In addition to the missing effect sizes at model and parameter levels, no effect size to date gives the researcher a feeling for potential replication of the invariant or non-invariant items. Last, in line with general suggestions by @cumming2012 and @cumming2016, the visualization of effect sizes in MGCFA would be an added tool for researchers to gauge the size of group differences.
