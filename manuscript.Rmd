...
Again, we see about a half-point difference between our clinical and student samples for item 6, which is about drive to achieve. The CFI for this model does meet the requirements for partial invariance, `r apa_p(glance(partial.rs.2)$cfi)`. The effect size is approximately the same at `r apa_num(lavaan_dmacs(partial.rs.2, "Clinical")$DMACS[6])`. Last, we can create our images to view the item non-invariance and the latent means in Figures \@ref(fig:rs6-img) and \@ref(fig:rs7-img).
...
```{r include = T, echo = T, results = "small", eval = F}
